...
namespace collision_detection
{
	...
	template<typename T, typename HitPolicy, typename CollitionDetectionPolicy, int MAX_TREE_LEVEL>
	void tree<T, HitPolicy, CollitionDetectionPolicy, MAX_TREE_LEVEL>::regist(T& object) {
	...
	private:
		...
		//実際に当たり判定を行いhitを適用
		void do_collision_detection(T* a,T* b) {
			if (CollisionDetection::collision_detection(*a, *b))
				Hit::hit(*a, *b);
		}

		//再帰して木の走査
		std::vector<T*> recursion_search_tree(std::vector<T*> stack, unsigned int spaceCellNum)
		{
